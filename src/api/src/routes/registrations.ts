...
import { CreateRegistrationSchema, SearchQuerySchema, UpdateStatusSchema } from "../lib/schemas";

export const registrationsRouter = new Hono();

// GET /registrations - 一覧取得・検索
registrationsRouter.get("/", zValidator("query", SearchQuerySchema), (c) => {
...
registrationsRouter.post("/", async (c) => {
  ...
  if (sealImageFile instanceof File && sealImageFile.size > 0) {
    ...
    if (!allowedTypes.includes(sealImageFile.type)) {
      return c.json(
        {
          error: "印影画像はJPEG、PNG、GIF、WebP形式でアップロードしてください",
        },
        400
      );
    }
...
registrationsRouter.patch("/:id/status", zValidator("json", UpdateStatusSchema), (c) => {
  const id = c.req.param("id");
